...
fileCanBeRead = function (dirPath, nameOfFileToRead) { 
  ...
  if ( 'try-error' %in% class(rd)) {
    ...
    message('OK: file successfully read into var: rd')
...
IsLaterDateAfterEarlierDate = function (LaterDate,EarlierDate){

    if(sum(LaterDate >= EarlierDate ) == length(rd$`Bet Result Date`)){
    message('OK: All Bet Result Dates are on or after Event dates')
  }else{
    message('Err: Not all Bet Result Dates are on or after Event dates')
  }
...
CorrDataTypes = function(){
  ...
  if (sum(CorrectDType) == number_of_cols ) {
    ...
    print(CorrectDType[FALSE]  )
    stop('Err: Column names and data types not as expected')
    }
}

# Check TrackNames are consistent 

AnyUnknownVenueNames = function(VenueNames){
knownVenueNames = c(  
  'Ballarat'
  ,'Bendigo'
  ,'Cranbourne'
...
if (length(VenueNames[!(VenueNames %in% knownVenueNames  )]) >0 ) {
  ...
  VenueNames[!(VenueNames %in% knownVenueNames  )]
} else {
